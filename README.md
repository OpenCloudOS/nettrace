...
> **分支说明**
> `master` 分支目前仅进行维护，不再新增功能。新功能主要在 `btf` 分支开发。`btf` 分支基于 `TRACING` 实现，性能与功能更强，目前已适配内核版本为 `6.6+`。
...
如果需要进行静态编译，只需在编译时添加`STATIC=1`参数即可：

```shell
make STATIC=1 all
```

静态编译会将所有依赖库链接到可执行文件中，生成的二进制文件可以不依赖系统的共享库直接运行。
